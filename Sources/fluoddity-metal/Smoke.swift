...
// ───────────────────────────────────────────────────────────────────────────
// Toolchain smoke test (foundation N=1).
//
...
//      required the NVIDIA-only GL_NV_shader_atomic_float extension. Native on
//      Apple Silicon. Each thread atomically adds 1.0 into one float; N < 2^24
//      so a correct run yields sum == N exactly.
// ───────────────────────────────────────────────────────────────────────────
func runSmoke() {
    ...
    do {
        ...
        print(String(format: "atomic_float : %.0f / %d expected   (%d threads)", sum, n, tgWidth))
        if sum == Float(n) {
